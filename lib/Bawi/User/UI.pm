...
sub new {
    ...
    my $cfg = new Bawi::User::Config(%arg);
    my $dbh = $cfg->DBName && $cfg->DBUser && $cfg->DBPasswd ?
            new Bawi::DBI(-cfg=>$cfg) : undef;
    my $lh = Bawi::User::L10N->get_handle() || undef;
    my $self = {
        cgi      => new CGI,
        template => undef,
        cfg      => $cfg, 
...
sub init {
    ...
    my $cfg = $self->cfg;
    my @path;
    #print STDERR "SkinDIR : ",$cfg->SkinDir,"\n";
...
sub is_mobile_device {
    ...
    return "iphone3" if $user_agent =~ m/iPhone OS 3/i;
    return "iphone2" if $user_agent =~ m/iPhone OS 2/i;
    return undef;
}
